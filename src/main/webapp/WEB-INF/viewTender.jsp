    ...
    <table class="table table-striped table-hover table-condensed table-bordered">
        ...
        <c:forEach var="item" items="${tenderSpec}">
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="navigatorInput" name="price" id="${item.getLotId()}" placeholder="0.00"
                           pattern="\d+(\.\d{2})?" form="offer"
                           value="<c:out value="${offer.get(item.getLotId()).getPrice()}"/>">
                </td>
                <td>Количество</td>
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="term_of_delivery" id="term" class="${item.getLotId()}" form="offer"
                           pattern="^[ 0-9]+$" required
                           value="<c:out value="${offer.get(item.getLotId()).getTermOfDelivery()}"/>">
                </td>
                <td>
                    <label for="condition">Условия оплаты</label>
                </td>
                <td>
                    <input type="text" name="condition_of_payment" id="condition" class="${item.getLotId()}" required
                           form="offer" value="<c:out value="${offer.get(item.getLotId()).getConditionOfPayment()}"/>">
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td>
                    <input list="incoterms" size="30" name="condition_of_delivery" class="${item.getLotId()}"
                           id="delivery" required form="offer"
                           value="<c:out value="${offer.get(item.getLotId()).getConditionOfDelivery()}"/>"
                           placeholder="Инкотермс 2010">
                ...
                <td>
                    <input type="number" min="0" max="10" step="0.1" name="guarantee" class="${item.getLotId()}"
                           id="guarantee" required form="offer" pattern="^[ 0-9]+$"
                           value="<c:out value="${offer.get(item.getLotId()).getGuarantee()}"/>">
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td colspan="3">
                    <textarea rows="5" cols="100" name="additional_info" id="info" form="offer"><c:out
                            value="${offer.get(item.getLotId()).getAdditionalInfo()}"/></textarea>
                </td>
            </tr>
            <tr>
                <td colspan="3"><input type="hidden" name="lot_id" value="${item.getLotId()}"></td>
            </tr>
