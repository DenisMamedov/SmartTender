        ...
        <tr>
            ...
            <c:if test="${sessionScope.get('user') != null}">
                <form method="post" action="?command=/viewTender&id=${item.getId()}">
                    <c:choose>
                    <c:when test="${requestScope.get('command') == '/'}"><button type="submit" class="btn btn-primary btn-lg active" value="${item.getId()}">Подать предложение</button></c:when>
                    <c:when test="${requestScope.get('command') == '/history'}"><button type="submit" class="btn btn-primary btn-lg active" value="${item.getId()}">Просмотреть предложение</button>
